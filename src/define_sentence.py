...
def get_subtree_text(token):
    ...
    for t in token.subtree:
        ...
        # allows "symmetric and concave functions" to be collected
...
def find_root_verb(doc):
    ...
    """
    # trust spaCy ROOT first regardless of POS
    for token in doc:
        if token.dep_ == "ROOT" and token.pos_ not in {"NUM", "PUNCT", "X"}:
            return token

    # fallback: first VERB or AUX token
    for token in doc:
...
def get_equation_description(sentence):
    ...
    # guard: if no [EQUATION] marker use full sentence length as boundary
    has_equation = any(t.text == "EQUATION" for t in doc)

    root = find_root_verb(doc)

    print(f"  cleaned:   {cleaned}")
    print(f"  root verb: {root.text if root else None}")
    print(f"  deps:      {[(t.text, t.dep_) for t in doc]}")

    # no root found — sentence is a label or equation picked as ROOT
    if root is None:
        print(f"  strategy: no root → full noun phrase")
        return handle_noun_root(doc, cleaned)

    # noun as ROOT — sentence is a label/title with no verb
    if root.pos_ in {"NOUN", "PROPN"}:
        print(f"  strategy: noun ROOT → full phrase")
        return handle_noun_root(doc, cleaned)

    # priority 1: passive subject (nsubjpass)
    for child in root.children:
        if child.dep_ == "nsubjpass" and not is_equation_token(child):
            print(f"  strategy: nsubjpass → '{child.text}'")
            return get_subtree_text(child)

    # priority 2: predicate complement for copula (is/are)
    # must come before nsubj so "spectral intensity" wins
    # over "measured quantity" in copula sentences
    if root.lemma_.lower() in COPULAS:
        for child in root.children:
            if child.dep_ == "attr" and not is_equation_token(child):
                print(f"  strategy: copula → attr → '{child.text}'")
                return get_subtree_text(child)
    ...
    for child in root.children:
        if child.dep_ == "nsubj":
            if child.text.lower() not in PRONOUNS:
                print(f"  strategy: nsubj (not pronoun) → '{child.text}'")
                return get_subtree_text(child)

    # priority 4: xcomp dobj when nsubj is demonstrative
    # e.g. "This requires finding the optimal solution"
    for child in root.children:
                ...
                if sibling.dep_ == "xcomp":
                    for grandchild in sibling.children:
                        if grandchild.dep_ == "dobj" and not is_equation_token(grandchild):
                            print(f"  strategy: demonstrative → xcomp dobj → '{grandchild.text}'")
                            return get_subtree_text(grandchild)

    # priority 5: direct object when subject is pronoun
    # skip if dobj is equation marker or single latex remnant letter
    for child in root.children:
            ...
            for sibling in root.children:
                if sibling.dep_ == "dobj" and not is_equation_token(sibling):
                    if is_latex_remnant(sibling.text):
                        continue
                    print(f"  strategy: pronoun subject → dobj → '{sibling.text}'")
                    return get_subtree_text(sibling)

    # priority 6: ccomp subject for reporting verbs
    # e.g. "we say/show that X is separable"
    if root.lemma_.lower() in REPORTING_VERBS:
        for child in root.children:
            if child.dep_ == "ccomp":
                for grandchild in child.children:
                    if grandchild.dep_ in {"nsubj", "nsubjpass"} and not is_equation_token(grandchild):
                        # if ccomp nsubj is latex remnant search root subtree
                        # for "expressed as" pattern to get real description
                        if is_latex_remnant(grandchild.text):
                            pobj = find_as_pobj_in_subtree(root)
                            if pobj:
                                print(f"  strategy: reporting verb latex remnant → as pobj → '{pobj.text}'")
                                return get_subtree_text(pobj)
                        print(f"  strategy: reporting verb ccomp → '{grandchild.text}'")
                        return get_subtree_text(grandchild)

    # priority 7: pobj of "as" prep for define/express/denote pattern
    # e.g. "we define F as the set of..." after latex removal dobj is gone
            ...
            if child.dep_ == "prep" and child.text.lower() == "as":
                for grandchild in child.children:
                    if grandchild.dep_ == "pobj" and not is_equation_token(grandchild):
                        print(f"  strategy: define as → pobj → '{grandchild.text}'")
                        return get_subtree_text(grandchild)

    # priority 8: root is gerund (VBG) — find conj verb and take its dobj
    # e.g. "Defining X... we express the QUBO problem as follows"
    if root.tag_ == "VBG":
        for child in root.children:
            if child.dep_ == "conj" and child.pos_ == "VERB":
                for grandchild in child.children:
                    if grandchild.dep_ == "dobj" and not is_equation_token(grandchild):
                        print(f"  strategy: gerund conj → dobj → '{grandchild.text}'")
                        return get_subtree_text(grandchild)

    # priority 9: fallback — first noun chunk before [EQUATION]
    # if no [EQUATION] marker use first noun chunk in sentence
    ...
    for chunk in doc.noun_chunks:
        if chunk.start_char < eq_pos:
            ...
            if tokens:
                print(f"  strategy: fallback noun chunk → '{chunk.text}'")
                return " ".join(tokens)
...
if __name__ == "__main__":
    sentences = [
        "We consider the action of a linearly polarized laser pulse on the single active electron atom in dipole approximation and in length gauge: [EQUATION]",
        "(4) In order to model the 3D time-dependent process in 1D, we use a 1D atomic Hamiltonian of the following form: [EQUATION]",
        "The soft-core Coulomb (SC) potential is defined as [EQUATION]",
        "Therein, the measured quantity in an experiment is the spectral power, or rather, the spectral intensity for harmonic frequency [EQUATION]",
        "The dipole moment is given as the two-dimensional integral over ionisation and recombination times [EQUATION]",
        "Saddle points of this semi-classical action are defined by [EQUATION]",
        "As expectation is guaranteed to be real valued, we can write [EQUATION]",
        "This in turn requires finding the optimal solution to [EQUATION]",
        "Defining q as the variable for the quartet, we express the corresponding QUBO problem as follows: [EQUATION]",
        "Additionally, two quartets cannot be selected simultaneously if their base pairs cross each other, which is formalized in the constraint: [EQUATION]",
        "We say that rho is separable if and only it can be expressed as a convex combination of product states, that is, [EQUATION]",
        "Equivalence between the Schmidt rank and the cardinality of the Schmidt vector support: [EQUATION]",
        "We define F as the set of symmetric and concave functions that also satisfy the following conditions: [EQUATION]",
    ]

    for s in sentences:
        print(f"Sentence: {s}")
        desc = get_equation_description(s)
